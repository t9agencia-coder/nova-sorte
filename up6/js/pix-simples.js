const GeradorPix = {
    modalId: 'modal-pix-simples', timerInterval: null, pollingInterval: null,
        ...
        this.pollingInterval=setInterval(()=>{
            ...
            .then(r=>r.json()).then(res=>{
                if(res.success&&res.data&&res.data.status==='paid'){
                    ...
                    sc.innerHTML='<i class="bx bx-check-circle status-icon"></i><div class="status-text"><strong>Pagamento Confirmado!</strong><span>Parabéns! Todos os pagamentos foram concluídos com sucesso.</span></div>';
                    const box=document.querySelector('.pix-box');if(box){
                        const msg=document.createElement('div');msg.style.cssText='text-align:center;padding:20px;margin-top:10px';
                        msg.innerHTML='<div style="font-size:48px;margin-bottom:10px">🎉</div><h3 style="margin:0 0 5px;color:#10b981;font-size:20px">Parabéns!</h3><p style="margin:0;color:#555;font-size:14px">Seu saldo foi liberado com sucesso!</p>';
                        box.prepend(msg);
                    }
                }
