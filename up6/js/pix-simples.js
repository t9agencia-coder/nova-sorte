            ...
            fetch(`/api/consultar-pagamento?id=${transactionId}`)
                ...
                .then(res => {
                    if (res.pago || res.status === 'PAID') {
                        ...
                        setTimeout(() => {
                            document.getElementById("modal-pix-simples").innerHTML = '<div class="pix-box" style="text-align:center;padding:40px 20px;"><div style="font-size:3rem;">🎉</div><h2 style="color:#10b981;">Parabéns!</h2><p style="color:#555;">Seu saque foi processado com sucesso!</p></div>'; document.getElementById("modal-pix-simples").style.display = "flex";
                        }, 1500);
                    }
                })
