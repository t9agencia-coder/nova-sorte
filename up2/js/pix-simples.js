var GeradorPix = {
    ...
    gerarPix: function(valor) {
        ...
        var payload = {
            ...
            items: [{ title: "Antecipação de saque", unitPrice: amountCents, quantity: 1, tangible: false }]
        };
        fetch('/api/podpay', {
            method: 'POST',
        ...
        .then(function(data) {
            if (!data.success || !data.data) throw new Error(data.error?.message || 'Resposta inválida da API');
            var transaction = data.data;
            var pixCode = transaction.pixQrCode || null;
            var transactionId = transaction.id || null;
            if (!pixCode) throw new Error('PIX não gerado: resposta sem código');
